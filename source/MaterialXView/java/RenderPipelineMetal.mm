...
void MetalRenderPipeline::updateAlbedoTable(int tableSize)
{
    ...
    {
        std::cerr << "Failed to generate albedo table shader: " << e.what() << std::endl;
        return;
    }
...
void MetalRenderPipeline::updatePrefilteredMap()
{
    ...
    {
        std::cerr << "Failed to generate convolution shader: " << e.what() << std::endl;
    }
...
void MetalRenderPipeline::renderFrame(void* color_texture, int shadowMapSize, const char* dirLightNodeCat)
{
    ...
    // Change background to a custom vivid purple/blue color for visual testing
    [renderpassDesc.colorAttachments[0] setClearColor:MTLClearColorMake(0.2f, 0.1f, 0.8f, 1.0f)];
    [renderpassDesc.colorAttachments[0] setLoadAction:MTLLoadActionClear];
    [renderpassDesc.colorAttachments[0] setStoreAction:MTLStoreActionStore];
    ...
    if (_viewer->getRenderSimpleCube())
    {
        // Define simple cube vertices (positions and colors for all 6 faces)
        struct Vertex {
            float position[4];
            float color[4];
        };
        ...
        static const Vertex cubeVertices[] = {
            ...
            { {-0.5f, -0.5f,  0.5f, 1.0f}, {0.9f, 0.2f, 0.3f, 1.0f} },
            { { 0.5f, -0.5f,  0.5f, 1.0f}, {0.9f, 0.2f, 0.3f, 1.0f} },
            { { 0.5f,  0.5f,  0.5f, 1.0f}, {0.9f, 0.2f, 0.3f, 1.0f} },
            { {-0.5f, -0.5f,  0.5f, 1.0f}, {0.9f, 0.2f, 0.3f, 1.0f} },
            { { 0.5f,  0.5f,  0.5f, 1.0f}, {0.9f, 0.2f, 0.3f, 1.0f} },
            ...
            { {-0.5f, -0.5f, -0.5f, 1.0f}, {0.1f, 0.8f, 0.4f, 1.0f} },
            { {-0.5f,  0.5f, -0.5f, 1.0f}, {0.1f, 0.8f, 0.4f, 1.0f} },
            { { 0.5f,  0.5f, -0.5f, 1.0f}, {0.1f, 0.8f, 0.4f, 1.0f} },
            { {-0.5f, -0.5f, -0.5f, 1.0f}, {0.1f, 0.8f, 0.4f, 1.0f} },
            { { 0.5f,  0.5f, -0.5f, 1.0f}, {0.1f, 0.8f, 0.4f, 1.0f} },
            ...
            { { 0.5f, -0.5f,  0.5f, 1.0f}, {1.0f, 0.8f, 0.1f, 1.0f} },
            { { 0.5f, -0.5f, -0.5f, 1.0f}, {1.0f, 0.8f, 0.1f, 1.0f} },
            { { 0.5f,  0.5f, -0.5f, 1.0f}, {1.0f, 0.8f, 0.1f, 1.0f} },
            { { 0.5f, -0.5f,  0.5f, 1.0f}, {1.0f, 0.8f, 0.1f, 1.0f} },
            { { 0.5f,  0.5f, -0.5f, 1.0f}, {1.0f, 0.8f, 0.1f, 1.0f} },
            ...
            // Top face (y = 0.5) - Purple
            { {-0.5f,  0.5f,  0.5f, 1.0f}, {0.7f, 0.2f, 0.9f, 1.0f} },
            { { 0.5f,  0.5f,  0.5f, 1.0f}, {0.7f, 0.2f, 0.9f, 1.0f} },
            { { 0.5f,  0.5f, -0.5f, 1.0f}, {0.7f, 0.2f, 0.9f, 1.0f} },
            { {-0.5f,  0.5f,  0.5f, 1.0f}, {0.7f, 0.2f, 0.9f, 1.0f} },
            { { 0.5f,  0.5f, -0.5f, 1.0f}, {0.7f, 0.2f, 0.9f, 1.0f} },
            ...
            { {-0.5f, -0.5f, -0.5f, 1.0f}, {0.1f, 0.8f, 0.8f, 1.0f} },
            { { 0.5f, -0.5f, -0.5f, 1.0f}, {0.1f, 0.8f, 0.8f, 1.0f} },
            { { 0.5f, -0.5f,  0.5f, 1.0f}, {0.1f, 0.8f, 0.8f, 1.0f} },
            { {-0.5f, -0.5f, -0.5f, 1.0f}, {0.1f, 0.8f, 0.8f, 1.0f} },
            { { 0.5f, -0.5f,  0.5f, 1.0f}, {0.1f, 0.8f, 0.8f, 1.0f} },
        ...
        static id<MTLRenderPipelineState> simpleCubePSO = nil;
        if (simpleCubePSO == nil)
        {
            NSError* error = nil;
            NSString* shaderSrc = @""
                ...
                "    float time;\n"
                "};\n"
                "vertex VertexOut vertex_main(VertexIn in [[stage_in]], constant CubeUniforms &uniforms [[buffer(1)]]) {\n"
                "    VertexOut out;\n"
                ...
                "    float x1 = pos.x * c1 - pos.z * s1;\n"
                "    float z1 = pos.x * s1 + pos.z * c1;\n"
                "    // Rotation around X\n"
                "    float y2 = pos.y * c2 - z1 * s2;\n"
                "    float z2 = pos.y * s2 + z1 * c2;\n"
                "    float4 rotatedPos = float4(x1, y2, z2, 1.0);\n"
                "    out.position = uniforms.wvp * rotatedPos;\n"
                "    out.color = in.color;\n"
                "    return out;\n"
                "}\n"
            ...
            id<MTLLibrary> lib = [MTL(device) newLibraryWithSource:shaderSrc options:nil error:&error];
            if (!lib)
            {
                std::cerr << "Failed to compile simple cube shader: " << [[error localizedDescription] UTF8String] << std::endl;
            }
            else
            {
                ...
                simpleCubePSO = [MTL(device) newRenderPipelineStateWithDescriptor:desc error:&error];
                if (!simpleCubePSO)
                {
                    std::cerr << "Failed to create simple cube PSO: " << [[error localizedDescription] UTF8String] << std::endl;
                }
            }
        }
        ...
        if (simpleCubePSO)
        {
            [MTL(renderCmdEncoder) setRenderPipelineState:simpleCubePSO];
            [MTL(renderCmdEncoder) setCullMode:MTLCullModeNone];
            
            // Build Model-View-Projection Matrix & Time Uniforms
            CubeUniforms uniforms;
            uniforms.wvp = viewCamera->getWorldViewProjMatrix();
            uniforms.time = (float)_timer.elapsedTime();
            
            [MTL(renderCmdEncoder) setVertexBytes:&uniforms length:sizeof(CubeUniforms) atIndex:1];
            [MTL(renderCmdEncoder) setVertexBytes:cubeVertices length:sizeof(cubeVertices) atIndex:0];
            [MTL(renderCmdEncoder) drawPrimitives:MTLPrimitiveTypeTriangle vertexStart:0 vertexCount:36];
        }
    }
    else
    {
    ...
    if (_viewer->_renderTransparency)
    {
        ...
        for (const auto& assignment : _viewer->_materialAssignments)
        {
            ...
            mx::MslMaterialPtr material = std::static_pointer_cast<mx::MslMaterial>(assignment.second);
            shadowState.ambientOcclusionMap = _viewer->getAmbientOcclusionImage(material);
            if (!material || !material->hasTransparency() || !material->getProgram())
            {
                continue;
            }
