...
class Viewer
{
  ...
  public:
    ...
    void draw_contents();
    void setColorTexture(void* tex) { _colorTexture = tex; }
    RenderPipelinePtr getRenderPipeline() { return _renderPipeline; }
    
    // Simple cube rendering toggle
    bool getRenderSimpleCube() const { return _renderSimpleCube; }
    void setRenderSimpleCube(bool enable) { _renderSimpleCube = enable; }
    
    // Size and background color helpers to replace Screen properties
    mx::Vector2 m_size;
    mx::Color4 m_background;
    float m_pixel_ratio;

    void set_background(const mx::Color4& color)
    {
        m_background = color;
    }
